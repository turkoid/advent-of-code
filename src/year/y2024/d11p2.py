...
*7 125 17
    ...
    def blink(self, index: int, count: int, stone: str, cache: dict[str, list[int]]) -> int:
        if index == count:
            return 0

        if index < len(cache.setdefault(stone, [])):
        ...
        if stone == "0":
            ...
            for split_stone in [stone[:split_point], stone[split_point:]]:
                stones.append(str(int(split_stone)))
        else:
            stones.append(str(int(stone) * 2024))
        added_stones = len(stones) - 1
        if added_stones == 0:
            cache[stone].append(added_stones)
        index += 1
        total = added_stones
        for stone in stones:
            total += self.blink(index, count, stone, cache)
        if added_stones == 1:
            cache[stone].append(added_stones)
        return total
    ...
    def solution(self, data: str) -> Any:
        ...
        if stones[0].startswith("*"):
            ...
            del stones[0]
        # self.print(0, stones=0)
        total = len(stones)
        for stone in stones:
            total += self.blink(0, count, stone, {})
        return total
