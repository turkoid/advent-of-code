...
class Plant:
    ...
    @property
    def north(self):
        return Fence.NORTH in self.fences

    @property
    def east(self):
        return Fence.EAST in self.fences

    @property
    def south(self):
        return Fence.SOUTH in self.fences

    @property
    def west(self):
        return Fence.WEST in self.fences
...
class Plot:
    ...
    def sides(self):
        if len(self.plants) < 3:
            return 4
        sides = 0
        edges = {p.location: p for p in self.plants if p.fences}
        offsets = {Fence.NORTH: (-1, 0), Fence.EAST: (0, -1), Fence.SOUTH: (-1, 0), Fence.WEST: (0, -1)}
        for x, y in sorted(edges.keys(), key=lambda p: (p[1], p[0])):
            plant = edges[x, y]
            for fence, offset in offsets.items():
                if fence in plant.fences:
                    location = (plant.x + offset[0], plant.y + offset[1])
                    if not (neighbor := edges.get(location, None)) or fence not in neighbor.fences:
                        sides += 1
        return sides
...
MMMISSJEEE
    ...
    def solution(self, data: str) -> Any:
        ...
        for id, plot in plots.items():
            ...
            sides = plot.sides
            cost = area * sides
            # print(f"{id[0]}{id[1]}: {area} * {sides} = {cost}")
            total += cost
        return total
